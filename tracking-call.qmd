...
total_trouve = int((data_cleaned["Trouvé"] == "Oui").sum()) if "Trouvé" in data_cleaned.columns else 0
...
:::
::: {.subtitle}
`{python} f"{total_trouve:,}"` trouvé(s) sur `{python} f"{total_interventions:,}"`
:::
:::
...
:::
::: {.subtitle}
`{python} (top_agent[:20] + "…") if top_agent and len(top_agent) > 20 else (top_agent if top_agent else "N/A")`
:::
:::
...
# 📈 Graphiques (1 graphique = 1 cadre)
...
if not data_cleaned.empty:
    if "Trouvé" in data_cleaned.columns:
        success_rate = (
            data_cleaned.groupby("day")["Trouvé"]
...
if not success_rate.empty:
    ...
    ax.set_title("Taux de Succès par Jour (%)")
...
if not visits.empty:
    ...
    plt.figure(figsize=(10,2))
    plt.text(0.5, 0.5, "Aucune donnée de visites.", ha="center", va="center")
    plt.axis("off")
    plt.tight_layout()
    plt.show()
```
...
if not calls.empty:
    ...
    plt.figure(figsize=(10,2))
    plt.text(0.5, 0.5, "Aucune donnée d'appels.", ha="center", va="center")
    plt.axis("off")
    plt.tight_layout()
    plt.show()
```
...
## Performance par agent (taux de succès)
...
#| fig-width: 10
#| fig-height: 6
if (not data_cleaned.empty) and ("username" in data_cleaned.columns) and ("Trouvé" in data_cleaned.columns):
    success_by_user = (
        data_cleaned.groupby("username")["Trouvé"]
        .apply(lambda x: (x == "Oui").mean() * 100)
        .round(0).astype(int)
        .reset_index(name="Success Rate (%)")
    ...
    ax.set_title("Taux de Succès par Agent (%)")
    ax.set_xlabel("Pourcentage (%)")
    ax.set_ylabel("Agent")
    for c in ax.containers:
        ax.bar_label(c, fmt="%d%%", padding=3)
    ...
    plt.figure(figsize=(10,2))
    plt.text(0.5, 0.5, "Données insuffisantes pour générer ce graphique.", ha="center", va="center")
    plt.axis("off")
    plt.tight_layout()
    plt.show()
```
