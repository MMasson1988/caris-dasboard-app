# Import functions from nutrition_pipeline.py for modularity
# Pipe-friendly function to assign age range from months
...
from nutrition_pipeline import (
    merge_with_depistage,
    select_columns,
    rename_cols,
    assign_age_range_from_months,
    print_message,
    capitalize_column,
    # Add any other functions you want to import from nutrition_pipeline.py
)
...
def merge_with_depistage(df, depistage_df, cols=None):
    ...
    if missing_cols:
        logger.warning(f"Missing columns in depistage_df: {missing_cols}")
    if not available_cols:
        logger.error("No common columns found for merge")
        return df
    logger.info(f"Merging on columns: {available_cols}")
    return pd.merge(df, depistage_df[available_cols], on='caseid', how='left')
def select_columns(df, columns):
    """Select a subset of columns from a DataFrame in a pipe-friendly way."""
    return df[columns]
# ===============================
...
def main():
    ...
    logger.info("Module nutrition_pipeline.py chargé. Ajoutez votre pipeline ici.")
...
def run_nutrition_pipeline():
    """
    Exécute tout le pipeline nutrition (modularisé).
    """
    #=========================================================================================================
    ...
    def save_to_excel(df, filename, index=False, **kwargs):
        ...
        logger.info(f"Nombre de patients dans le fichier {filename}: {df.shape[0]}")
    ...
    # ...existing code...
    # (Insert all the rest of the script's logic here, indented under run_nutrition_pipeline())
    # ...existing code...

    # (The rest of the script is unchanged, but indented under run_nutrition_pipeline())

    return df
    """
    Point d'entrée du script nutrition : lance le pipeline principal.
    """
    run_nutrition_pipeline()
...
def extract_user_mamba(df, username_col='username', output_col='user_mamba'):
    ...
    if username_col in df.columns:
        ...
        logger.warning(f'{username_col} column not found')
...
def fill_office_from_data_clerk(df, data_clerk_col='data_clerk', office_col='office'):
    ...
    if data_clerk_col not in df.columns:
        logger.warning(f'{data_clerk_col} column not found')
        return df
    ...
    logger.info(f"✅ Created office column based on data_clerk mapping:")
    return df
...
def create_status_column(df, col1, col2, col3, threshold=0, operator='!=', 
    ...
    df.loc[:, output_col] = np.where(condition, true_value, false_value)
    logger.info(f"Colonne '{output_col}' créée:")
    logger.info(f"  - {true_value}: {(df[output_col] == true_value).sum()}")
    logger.info(f"  - {false_value}: {(df[output_col] == false_value).sum()}")
    return df
...
def filter_enrolled_patients(
    ...
    logger.info(f"Nombre d'enrollement avec doublons possibles {filtered_df.shape[0]} lignes")
...
def create_backup_and_continue(df, backup_var_name="backup"):
    ...
    logger.info(f"✅ Backup créé: {backup_var_name}")
    return df
...
def create_mamba_period_column(df, user_mamba_col='user_mamba', date_col='date_enrollement', 
    ...
    logger.info(f"Colonne {output_col} créée:")
    logger.info(df[output_col].value_counts())
...
logger.info(f"✓ Dossier de sortie vérifié: {output_dir}")
...
#=========================================================================================================
logger.info("="*60)
logger.info(f"DEBUT DE LA PIPELINE DE NUTRITION à {today_str}")
logger.info(f"DE {start_date} à {end_date}")
logger.info("="*60)
#=====================================================================================================================
...
logger.info(f"Chemin du fichier de dépistage: {depistage_file}")
logger.info(f"Le fichier existe-t-il? {os.path.exists(depistage_file)}")
...
#==========================================================================================================
logger.info("=== TRAITEMENT DES DONNÉES DE SUIVI ===")
...
logger.info(f"Agrégation terminée: {suivi_aggregated.shape[0]} patients uniques avec {suivi_aggregated.shape[1]} colonnes")
suivi_aggregated.to_excel(os.path.join(output_dir, 'suivi_aggregated_complete.xlsx'), index=False)
logger.info(f"✅ Fichier sauvegardé: {os.path.join(output_dir, 'suivi_aggregated_complete.xlsx')}")
logger.info(f"✅ Toutes les {suivi_aggregated.shape[1]} colonnes ont été conservées avec l'agrégation appropriée")
...
if missing_cols:
    logger.warning(f"⚠️  Colonnes manquantes pour raison_pour_la_sortie: {missing_cols}")

if existing_cols:
    logger.info(f"✓ Colonnes trouvées pour raison_pour_la_sortie: {existing_cols}")
    ...
    logger.info(f"✅ Colonne 'raison_pour_la_sortie' créée:")
    logger.info(suivi_aggregated['raison_pour_la_sortie'].value_counts())
else:
    logger.warning("❌ Aucune colonne de raison trouvée, colonne 'raison_pour_la_sortie' non créée")
...
logger.info(f"✅ Fichier final sauvegardé: {os.path.join(output_dir, 'suivi_aggregated_final.xlsx')}")
#=========================================================================================================
#===============================================================================================
...
logger.info(f"✅ Séparation effectuée:")
logger.info(f"  - Patients enrôlés à caris_itu: {nut_filtered_caris_itu.shape[0]}")
logger.info(f"  - Patients enrôlés en community: {nut_filtered_community.shape[0]}")
...
logger.info(f"  - {os.path.join(output_dir, f'enrolés_caris_itu.xlsx')}")
logger.info(f"  - {os.path.join(output_dir, f'enrolés_community.xlsx')}")
...
logger.info(f"✅ Filtrage par approval_date effectué:")
logger.info(f"  - Patients approuvés entre {start_date_nut.date()} et {end_date_nut.date()}: {aproval_filtered.shape[0]}")
...
logger.info(f"Debug: nut_filtered_caris_final shape: {nut_filtered_caris_final.shape}")
logger.info(f"Debug: suivi_enroled shape: {suivi_enroled.shape}")
...
logger.info(f"✅ Concatenation successful: {enroled_total.shape[0]} total rows")
logger.info(f"✅ Remplissage des valeurs NA pour nut_filtered_caris_final:")
logger.info(f"  - Avant remplissage: {nut_filtered_caris_final.shape[0]} patients")
...
for col in numeric_cols:
    if col in enroled_total.columns:
        ...
        logger.info(f"✅ Colonne {col} remplie avec 0")
    else:
        logger.warning(f"⚠️ Colonne {col} manquante dans enroled_total")
...
logger.info(f"✅ Remplissage des NA terminé pour nut_filtered_caris_final")

# Créer la colonne 'enroled' selon les conditions spécifiées
logger.info("=== CRÉATION DE LA COLONNE ENROLED ===")
...
logger.info("=== LES ELIGIBLES EN ATTENTE ===")
...
logger.info(f"Nombre d'éligibles sans suivi: {eligible_sans_suivi.shape[0]}")
...
logger.info(f"Nombre d'éligibles après filtrage: {eligible_filtered.shape[0]}")

logger.info("=== CRÉATION DE LA COLONNE EN_ATTENTE ===")
...
if missing_cols_attente:
    logger.warning(f"⚠️  Colonnes manquantes pour en_attente: {missing_cols_attente}")
...
if 'approval_date' not in eligible_filtered.columns:
    ...
    logger.info("✓ Colonne 'approval_date' créée avec valeur par défaut '---'")
...
logger.info(f"✅ Colonne 'en_attente' créée:")
logger.info(f"  - en_attente = yes: {(eligible_filtered['en_attente'] == 'yes').sum()}")
logger.info(f"  - en_attente = no: {(eligible_filtered['en_attente'] == 'no').sum()}")
...
logger.info("=== LES CLUBS DE NUTRITION ===")
...
logger.info(f"Chemin du fichier de club: {club_file_path}")
logger.info(f"Le fichier existe-t-il? {os.path.exists(club_file_path)}")
...
logger.info(f"Nombre de clubs avec date de session future: {clubs_future.shape[0]}")
if not clubs_future.empty:
    logger.warning("⚠️  Attention: Des sessions de club ont des dates futures:")
    save_to_excel(
        clubs_future, 
    ...
    logger.info("✓ Aucune session de club avec date future trouvée.")
...
logger.info(f"Agrégation terminée: {club_aggregated.shape[0]} patients uniques avec {club_aggregated.shape[1]} colonnes")
club_aggregated.to_excel(os.path.join(output_dir, 'club_nutrition.xlsx'), index=False)
logger.info(f"✅ Fichier sauvegardé: {os.path.join(output_dir, 'club_nutrition.xlsx')}")
logger.info(f"✅ Toutes les {club_aggregated.shape[1]} colonnes ont été conservées avec l'agrégation appropriée")
# File club
...
if __name__ == '__main__':
    logger.info("Module nutrition_pipeline.py chargé. Ajoutez une fonction main() pour exécuter le pipeline.")