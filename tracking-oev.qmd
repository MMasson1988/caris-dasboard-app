...
def plot_viral_load_summary2_sns(df, age_min=0, age_max=18, start_date=None, end_date=None, title=None, output='plot', palette='viridis'):
    ...
    # end_date = dernier jour du mois précédent
    # start_date = 1er jour, 13 mois avant le mois précédent
    today = datetime.today()
    if end_date is None:
        end_date = (today.replace(day=1) - pd.Timedelta(days=1))  # Dernier jour du mois précédent
    else:
    ...
    # 4. Pas de résultat (Eligible mais pas de données)
...
tx_curr.to_csv("outputs/OEVtx_curr_for_r.csv", index=False)
```
...
```{r, echo=FALSE}
rtx_curr <- read.csv("outputs/OEVtx_curr_for_r.csv", stringsAsFactors = FALSE)
...
# Save to CSV for R
not_covered.to_csv("outputs/OEVnot_covered_for_r.csv", index=False)
no_result.to_csv("outputs/OEVno_result_for_r.csv", index=False)
vl_coverage.to_csv("outputs/OEVvl_coverage_for_r.csv", index=False)
vl_coverage_cor.to_csv("outputs/OEVvl_coverage_cor_for_r.csv", index=False)
```

## Enfants sans charges virales actualisées
```{r, echo=FALSE}
rnot_covered <- read.csv("outputs/OEVnot_covered_for_r.csv", stringsAsFactors = FALSE)
...
```{r,echo=FALSE}
rno_result <- read.csv("outputs/OEVno_result_for_r.csv", stringsAsFactors = FALSE)
...
graf_abs_pres_oev$present<-ifelse(graf_abs_pres_oev$present=='1','présence','absence')
...
:::card
Durant cette période, sur `r sum(graf_abs_pres_oev$freq)` enfants oev attendues en club, `r pres_oev$freq` était(aient) présent(s) contre `r abs_oev$freq` qui était(aient) absent(s).
:::
