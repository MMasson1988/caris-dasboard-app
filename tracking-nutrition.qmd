    ...
    include-in-header:
      - text: |
          <style>
            ...
            /* Évite que le titre soit caché par un éventuel header fixe */
          ...
          document.addEventListener('DOMContentLoaded', function() {
              ...
              // 2. Génération de la TOC dynamique pour les graphiques (H4)
              function buildDynamicToc() {
                  ...
                  tabPanes.forEach(pane => {
                      ...
                      ul.style.display = 'none'; // Caché par défaut

                      h4s.forEach(h4 => {
                          const li = document.createElement('li');
                          const a = document.createElement('a');
                          // On récupère l'ID généré par Quarto ou on en crée un
              ...
              // 3. Mise à jour de la visibilité selon l'onglet
              ...
              setTimeout(() => {
                  ...
                  // Écouteur sur les changements d'onglets Bootstrap
...
reticulate::use_virtualenv("C:/Users/moise/Downloads/caris-meal-app/.venv", required = TRUE)
```
...
# NOTE IMPORTANTE : Pour que les jours s'affichent en français (Lundi, Mardi, etc.),
# vous devez vous assurer que la locale française est configurée dans votre environnement.
...
def plot_daily_data_weekly_ordered(df, date_column, plot_title):
    """
    Génère un histogramme horizontal des cas par jour pour une seule semaine.
    Affiche SEULEMENT le nom du jour sur l'axe Y et respecte l'ordre chronologique.

    :param df: DataFrame contenant les données (max 7 jours).
    :param date_column: Nom de la colonne contenant les dates.
    :param plot_title: Titre du graphique.
    """
    
    # 1. Préparation des données (Conversion et Ordre Chronologique)
    df[date_column] = pd.to_datetime(df[date_column], errors='coerce')
    
    # Étiquette de l'axe Y : Nom du jour seul (ex: Lundi)
    ...
    # Groupement par l'ordre (pour le tri) et l'étiquette (pour l'affichage)
    ...
    # 5. Ajout des annotations (valeurs numériques)
    for bar in bars:
        width = bar.get_width()
...
def plot_percentage_enhanced(df, category_col, value_col, target_col, color_no=None, color_yes=None):
    """
    Crée un graphique à barres empilées de fréquences/comptes.
    Affiche la somme brute, le pourcentage, le nom du secteur dans chaque section,
    et le total de chaque catégorie au-dessus des barres.
    Supprime les gridlines et utilise des couleurs personnalisées ou par défaut.

    Args:
        df (pd.DataFrame): Le DataFrame à utiliser (ex: tips).
        category_col (str): La colonne contenant les catégories à tracer (ex: 'day').
        value_col (str): La colonne contenant les valeurs à compter (ex: 'total_bill').
        target_col (str): La colonne binaire pour la proportion (ex: 'smoker').
        color_no (str, optional): Couleur pour la catégorie 'No' (secteur du haut).
                                          Par défaut, utilise une couleur foncée.
        color_yes (str, optional): Couleur pour la catégorie 'Yes' (secteur du bas).
                                          Par défaut, utilise une couleur claire.
    """
    ...
    # Couleurs par défaut basées sur les couleurs de votre graphique de référence
    ...
    # 1. Barre de fond (Target 'No', couleur foncée)
    sns.barplot(
    ...
    ax.set_title(f'Répartition des Comptes par {category_col.replace("_", " ").title()} et {target_col.title()}', fontsize=14)
    ...
    ax.spines['left'].set_visible(False) # Souvent souhaité pour les bar charts
    ax.spines['bottom'].set_visible(False) # Souvent souhaité pour les bar charts
...
def plot_beneficiaries_stacked_matplotlib1(
        ...
        set_title: str = "Répartition bénéficiaires (eligible / non eligible)",
        set_xlabel: str = "Nombre de bénéficiaires",
        set_ylabel: str = "Catégorie",
        horizontal: bool = True,
        table: str = "no",           # "yes" => retourne seulement tableau
        percent: bool = False,       # True → tableau et graph en %
        display: str = "plot"        # "plot" | "table" | "both"
    ):
    """
    Crée un graphique à barres empilées Matplotlib et une table stylisée Pandas.
    La logique d'affichage est corrigée pour éviter l'espace blanc indésirable.
    """
    ...
    # --- SECTION CORRIGÉE POUR L'AFFICHAGE MULTIPLE ---
    if display == "both":
        # 1. Affiche le graphique (via plt.show())
        plt.show()
        # 2. Retourne le tableau stylisé. Quarto/knitr affiche ceci comme une sortie séparée
        #    juste après le graphique, sans l'espace blanc généré par ipython_display()
        return styled_table
...
def plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        group_col: str = "username",
        eligible_col: str = "eligible",
        plot_col: str = None,        # La colonne à sommer ou compter
        sum_logic: int = 0,          # 1 = Somme numérique, 0 = Comptage (Fréquence)
        set_title: str = "Répartition bénéficiaires",
    ...
    """
    Génère un graphique et un tableau. 
    Gère soit la somme d'une colonne numérique, soit le comptage d'occurrences.
    """
    ...
    if sum_logic == 1:
        ...
        if plot_col is None:
            raise ValueError("plot_col doit être spécifié si sum_logic=1")
...
def plot_time_data(df, date_column, plot_title, period='M'):
    """
    Génère un histogramme horizontal basé sur la période spécifiée
    ...
    Args:
        ...
        period (str): La période de regroupement ('S' pour Semaine, 'M' pour Mois, 'T' pour Trimestre).
    """
    
    # --- 1. Préparation des Données et Regroupement ---
    df = df.copy() # Travail sur une copie
    df[date_column] = pd.to_datetime(df[date_column], errors='coerce')
    
    # --- 2. Logique spéciale pour les semaines ---
    if period == 'S':
        # Pour éviter les problèmes avec les semaines partielles dans un mois filtré,
        # on groupe par semaine ISO (numéro de semaine dans l'année)
        df['year'] = df[date_column].dt.year
        df['week'] = df[date_column].dt.isocalendar().week
        df['week_start'] = df[date_column] - pd.to_timedelta(df[date_column].dt.dayofweek, unit='d')
        
        # Grouper par début de semaine
        ...
        def format_week_simple(row):
            ...
            # Affichage simplifié : seulement les dates (suppression du numéro de semaine)
            start_day = start.day
        ...
        if period not in freq_map:
            raise ValueError("L'argument 'period' doit être 'S' (Semaine), 'M' (Mois) ou 'T' (Trimestre).")
    ...
    # Ajout des valeurs numériques sur les barres (taille augmentée)
    for bar in bars:
        width = bar.get_width()
...
display_table <- function(df, 
                               table_title = "Répartition des Enfants par Club", 
                               header_color = "#800000") {
  
  # 1. Vérification : le dataframe existe-t-il et contient-il des données ?
  if (!is.null(df) && nrow(df) > 0) {
    
    # Préparation du titre avec la date du jour
    full_export_title <- paste(table_title, "-", format(Sys.Date(), "%d/%m/%Y"))
    
    # 2. Génération de la table DT
    DT::datatable(
      df,
      extensions = 'Buttons',
      filter = 'bottom',
...
#### Nombre de dépistage par jour pour la semaine écoulée {#depistage-semaine}
```{python, echo=FALSE}
...
if not df_week.empty :
    plot_daily_data_weekly_ordered(
        ...
        plot_title=f'Nombre de dépistage pour la semaine\n Total : {df_week.shape[0]}'
    )
else :
    print("Pas de données disponibles")
```

#### Liste des dépistages pour la semaine {#liste-depistage-semaine}
```{r, echo=FALSE}
display_table(py$df_week, "Dépistages_semaine", "#000080")
```

#### Nombre de dépistages pour la semaine par bureau {#depistage-semaine-agent}
```{python, echo=FALSE}
if not df_week.empty :
    plot_beneficiaries_by_categorie(
        df=df_week,
        lo_department='username',
        set_title='Nombre de dépistages en nutrition par bureau',
        set_xlabel='Dépistages',
        set_ylabel='bureau'
    )
else :
    print("Pas de données disponibles")
...
#### Nombre d'éligible pour la semaine par agent {#eligible-semaine-agent}
```{python, echo=FALSE}
...
if not df_week_eligible.empty:
    plot_beneficiaries_by_categorie(
        df=df_week_eligible,
        lo_department='username',
        set_title='Nombre de beneficiaires éligibles en nutrition par agent',
...
#### Nombre d'éligible pour la semaine par agent {#eligible-semaine-type}
```{python, echo=FALSE}
if not df_week_eligible.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
            ...
            set_title = "Répartition des éligibles par type de malnutrition",
            set_xlabel = "Nombre d'éligibles",
            set_ylabel = "Type de malnutrition",
            horizontal = True,
            table = "no",           # "no" => permet d'afficher le graphique
            percent = False,       # True → tableau et graph en %
...
#### Nombre d'enrolés pour la semaine par agent
```{python, echo=FALSE}
...
#### liste des enfants enrolés pour la semaine {#liste-enroled-semaine}
```{r, echo=FALSE}
display_table(py$enroled_week, "Enfants_enrolés", "#000080")
```


#### Nombre d'enfants à exeater par agent {#child-to-exit-semaine}
```{python, echo=FALSE}
...
if not child_to_exit_week.empty and 'username' in child_to_exit_week.columns:
    plot_beneficiaries_by_categorie(
        ...
        lo_department='username',
        set_title='Nombre d\'enfants à exeater par agent',
...
#### Liste des enfants à exeater pour la semaine {#liste-child-to-exit-semaine}
```{r, echo=FALSE}
display_table(py$child_to_exit_week, "Enfants_a_exeater", "#000080")
```

#### Nombre d'enfants exéatés pour la semaine par agent {#exeat-semaine-agent}
```{python, echo=FALSE}
...
if not exeat_week.empty and 'username' in exeat_week.columns:
    plot_beneficiaries_by_categorie(
        df=exeat_week,
        lo_department='username',
        set_title='Nombre d\'enfants exéatés par agent',
        set_xlabel='Enfants exéatés',
        set_ylabel='Agent'
    )
else:
    print("Pas de données disponibles")
...
#### Liste des enfants exéatés pour la semaine {#liste-exeat-semaine}
```{r, echo=FALSE}
display_table(py$exeat_week, "Enfants_exeatés", "#000080")
```
...
:::{.card}
:::{.card-header}
📊 **Clubs de Nutrition - Analyse Hebdomadaire**
:::
...
#nombre_children_en_club_week = club_week['caseid'].nunique()

# Calcul du nombre d'enfants uniques par club (caseid) en gardant les colonnes demandées
if not club_week.empty:
    children_uniques_par_club = club_week.groupby(['club_id', 'member_type', 'first_session_date', 'last_session_date', 'data_clerk']).agg(nombre_enfants_uniques=('caseid', 'nunique'))
else :
    print("Pas de données disponibles")
...
if not children_by_club.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        set_title = "Répartition des enfants en club par malnutris vs voisins",
...
*Données pour la période : semaine précédente*
:::

:::{.card}
:::{.card-header}
📝 **Note sur le nombre d'enfants en club**
:::
:::{.card-body}
Le nombre d'enfants en club correspond au nombre d'enfants uniques inscrits dans chaque club pour la période analysée. Ce calcul exclut les clubs de test et regroupe les enfants par club, type de membre, date de première et dernière session, et agent responsable. Cela permet d'obtenir une vision précise de la fréquentation réelle des clubs actifs.

```{python, echo=FALSE}
if 'nombre_children_en_club_week' in globals() and nombre_children_en_club_week > 0:
    print(f"{nombre_children_en_club_week} enfants ont participé aux clubs de nutrition cette semaine écoulée\nEt {nombre_clubs_uniques} clubs étaient actifs.")
```
:::
:::
:::
...
if not attente_week.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        eligible_col = "manutrition_type",
        set_title = "Répartition des enfants en en attente par type de malnutrition",
...
#### Récapitulatif de la semaine écoulée {#recapitulatif-semaine-ecoulee}

:::{.card}
:::{.card-header}
📈 **Tableau de bord - Semaine Écoulée**
:::

:::{.card-body}
```{python, echo=FALSE}
# Préparation des métriques clés pour un affichage moderne
import pandas as pd
...
# Création du DataFrame récapitulatif simple
summary_data = []

if metrics['depistages'] > 0:
    summary_data.append({
        'Indicateur': 'Dépistages réalisés',
...
if summary_data:
    ...
    # Méthodes d'affichage du DataFrame :
    
    # Option 1 : Affichage simple avec print (texte brut)
    print("=== Récapitulatif de la semaine ===")
    #print(summary_df.to_string(index=False))
    
    # Option 2 : Affichage formaté en HTML (plus joli)
...
#### Nombre de dépistage par jour pour la semaine courante {#depistage-semaine-courante}
```{python, echo=FALSE}
...
if not df_current_week.empty :
    plot_daily_data_weekly_ordered(
        df=df_current_week,
        date_column='date_de_depistage',
        plot_title=f'Nombre de dépistage pour la semaine courante\n Total : {df_current_week.shape[0]}'
    )
else :
    print("Pas de données disponibles")
```

#### Liste des dépistages pour la semaine courante {#liste-depistage-semaine-courante}
```{r, echo=FALSE}
display_table(py$df_current_week, "Dépistages_semaine", "#000080")
```

#### Nombre de dépistages pour la semaine courante par  {#depistage-semaine-courante-agent}
```{python, echo=FALSE}
if not df_current_week.empty :
    plot_beneficiaries_by_categorie(
        df=df_current_week,
        lo_department='username',
        set_title='Nombre de dépistages en nutrition par agent',
        set_xlabel='Beneficiaires',
        set_ylabel='agent'
    )
else :
...
#### Nombre d'éligible pour la semaine courante par agent {#eligible-semaine-courante-agent}
```{python, echo=FALSE}
...
#### Nombre d'éligible pour la semaine courante par agent {#eligible-semaine-courante-type}
```{python, echo=FALSE}
if not df_current_week_eligible.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
            df=df_current_week_eligible,
...
#### Nombre d'enrolés pour la semaine courante par agent {#enroled-semaine-courante-agent}
```{python, echo=FALSE}
...
if not enroled_current_week.empty :
    plot_beneficiaries_by_categorie(
        ...
        lo_department='username',
        set_title='Nombre d\'enrolés en nutrition par agent',
        set_xlabel='Beneficiaires',
        set_ylabel='agent'
    )
else :
...
#### liste des enfants enrolés pour la semaine courante {#liste-enroled-semaine-courante}
```{r, echo=FALSE}
display_table(py$enroled_current_week, "Enfants_enrolés", "#000080")
```


#### Nombre d'enfants à exeater par agent {#child-to-exit-semaine-courante}
```{python, echo=FALSE}
...
if not child_to_exit_current_week.empty and 'username' in child_to_exit_current_week.columns:
    ...
    print("Pas de données de sortie par agent")
```

#### Liste des enfants à exeater pour la semaine courante {#liste-child-to-exit-semaine-courante}
```{r, echo=FALSE}
display_table(py$child_to_exit_current_week, "Enfants_a_exeater", "#000080")
```

#### Nombre d'enfants exéatés pour la semaine courante par agent {#exeat-semaine-courante-agent}
```{python, echo=FALSE}
...
#### Liste des enfants exéatés pour la semaine courante {#liste-exeat-semaine-courante}
```{r, echo=FALSE}
display_table(py$exeat_current_week, "Enfants_exeatés", "#000080")
```
...
# 2. Vérification si les données filtrées existent
if not club_current_week.empty:
    ...
    # Préparation de club_create (Renommage si nécessaire)
    if 'caseid' in club_create.columns:
    ...
    # --- LE POINT DE CONTRÔLE CRITIQUE ---
    # On ne lance le plot que si le dataframe final après fusion et exclusion n'est pas vide
    ...
    if not children_by_current_club.empty:
        plot_beneficiaries_stacked_eligible_matplotlib(
            ...
            eligible_col = "member_type",
            set_title = "Répartition des enfants en club par malnutris vs voisins (Semaine Courante)",
            set_xlabel = "Nombre d'enfants en club",
            set_ylabel = "Type de membre",
            horizontal = True,
            table = "no",
        ...
        print("Pas de données disponibles après filtrage des clubs.")
        children_by_current_club = pd.DataFrame() # Évite les erreurs dans les blocs suivants
else:
    print("Aucune session enregistrée pour la semaine courante.")
...
if 'nombre_children_en_club_current' in globals() and nombre_children_en_club_current > 0:
    print(f"{nombre_children_en_club_current} ont participé aux clubs de nutrition la semaine courante\nEt {nombre_clubs_current} clubs étaient actifs.")
```
:::
:::
:::
...
#### Nombre d'enfants en attente pour la semaine écoulée par agent {#attente-semaine-courante}
```{python, echo=FALSE}
...
#### Liste des enfants en attente pour la semaine écoulée {#liste-attente-semaine-courante}
```{r, echo=FALSE}
display_table(py$attente_current_week, "Enfants_en_attente", "#000080")
```

#### Récapitulatif de la semaine écoulée {#recapitulatif-courante}
...
if summary_data:
    ...
    # Option 1 : Affichage simple avec print (texte brut)
    print("=== Récapitulatif de la semaine écoulée ===")
    #print(summary_df.to_string(index=False))
...
#### Nombre de dépistages par jour pour le mois précédent {#depistage-mois}
```{python, echo=FALSE}
...
if not df_month.empty:
    plot_time_data(
        df=df_month,
        date_column='date_de_depistage',
        plot_title=f'Nombre de dépistages pour le mois précédent\n Total : {df_month.shape[0]}',
        period='S'
    )
else:
    print("Pas de données disponibles")
...
#### Nombre de dépistage par jour pour le mois précédent {#depistage-daily-month}
```{python, echo=FALSE}
if not df_month.empty:
    plot_daily_data_weekly_ordered(
        df=df_month,
        date_column='date_de_depistage',
        plot_title=f'Nombre de dépistage pour le mois\n Total : {df_month.shape[0]}'
    )
else :
    print("Pas de données disponibles")
```

#### Liste des dépistages pour le mois précédent {#liste-depistage-mois}
```{r, echo=FALSE}
display_table(py$df_month, "Dépistages_mois", "#800000")
```

#### Nombre de dépistages par bureau pour le mois précédent  {#depistage-mois-agent}
```{python, echo=FALSE}
if not df_month.empty :
    plot_beneficiaries_by_categorie(
        df=df_month,
...
#### Nombre d'éligible pour la semaine par agent {#eligible-month-office}
```{python, echo=FALSE}
...
if not df_month_eligible.empty:
    plot_beneficiaries_by_categorie(
        ...
        set_title='Nombre de beneficiaires éligibles en nutrition par office',
        set_xlabel='Beneficiaires',
        set_ylabel='office'
    )
else:
...
#### Nombre d'éligible pour le mois précédent par agent {#eligib-month-type}
```{python, echo=FALSE}
if not df_month_eligible.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
            df=df_month_eligible,
...
#### Nombre d'enrolés pour le mois précédent par agent {#enroled-mois-agent}
```{python, echo=FALSE}
...
#### liste des enfants enrolés pour le mois précédent {#liste-enroled-month}
```{r, echo=FALSE}
display_table(py$enroled_month, "Enfants_enrolés_mois", "#000080")
```


#### Nombre d'enfants à exeater par bureau {#child-to-exit-month}
```{python, echo=FALSE}
...
if not child_to_exit_month.empty and 'username' in child_to_exit_month.columns:
    plot_beneficiaries_by_categorie(
        ...
        set_title='Nombre d\'enfants à exeater par bureau',
        set_xlabel='Nombre d\'enfants',
        set_ylabel='Bureau'
    )
else:
...
#### Liste des enfants à exeater pour le mois précédent {#liste-child-to-exit-month}
```{r, echo=FALSE}
display_table(py$child_to_exit_month, "Enfants_à_exeater_mois", "#800000")
```

#### Nombre d'enfants exéatés pour le mois précédent par agent {#exeat-month-agent}
```{python, echo=FALSE}
...
if not exeat_month.empty and 'office' in exeat_month.columns:
    plot_beneficiaries_by_categorie(
        ...
        set_title='Nombre d\'enfants exéatés par office',
        set_xlabel='Enfants exéatés',
        set_ylabel='Office'
    )
else:
    print("Pas de données de sortie par office")
```

#### Liste des enfants exéatés pour le mois précédent {#liste-exeat-mois}
```{r, echo=FALSE}
display_table(py$exeat_month, "Enfants_exéatés_mois", "#000080")
```

#### Nombre d'enfants en club Nutrition pour le mois précédent par agent {#club-nutrition-mois-agent}

:::{.card}
:::{.card-header}
📊 **Clubs de Nutrition - Analyse Mensuelle**
:::
...
if not club_month.empty:
    children_uniques_par_club = club_month.groupby(['club_id', 'member_type', 'first_session_date', 'last_session_date', 'data_clerk']).agg(nombre_enfants_uniques=('caseid', 'nunique'))
else :
    print("Pas de données disponibles")
...
:::{.card-footer .text-muted}
*Données pour la période : mois précédent*
:::
...
if 'nombre_children_en_club_month' in globals() and nombre_children_en_club_month > 0:
    print(f"{nombre_children_en_club_month} enfants ont participé aux clubs de nutrition le mois précédent.\nEt {nombre_clubs_month} clubs étaient actifs.")
```
:::
:::
:::

#### Liste des enfants inscrits en club pour le mois précédent {#liste-children-by-club-mois}
```{r, echo=FALSE}
...
#### Liste des enfants en attente pour le mois précédent {#liste-attente-mois}
```{r, echo=FALSE}
display_table(py$attente_month, "Enfants_en_attente_mois", "#000080")
```

#### Récapitulatif du mois précédent {#recapitulatif-month}

:::{.card}
:::{.card-header}
📈 **Tableau de bord - Mois Précédent**
:::
...
if summary_data:
    ...
    # Option 1 : Affichage simple avec print (texte brut)
    print("=== Récapitulatif du mois précédent ===")
    #print(summary_df.to_string(index=False))
...
#### Nombre de dépistages {#depistage-global-jours}
...
if not depistage_filtered.empty:
    plot_time_data(
        ...
        date_column='date_de_depistage',
        plot_title=f'Nombre de dépistages\n Total : {depistage_filtered.shape[0]}',
        period='M'
    )
else:
    print("Pas de données disponibles")
...
#### Nombre de dépistage par jour pour la periode {#depistage-daily-global}
```{python, echo=FALSE}
if not depistage_filtered.empty:
    plot_daily_data_weekly_ordered(
        df=depistage_filtered,
...
#### Liste des dépistages pour la période sélectionnée {#liste-depistage-periode}
```{r, echo=FALSE}
display_table(py$depistage_filtered, "Dépistages_periode", "#800000")
```

#### Nombre de dépistages par bureau pour la periode {#depistage-periode-agent}
```{python, echo=FALSE}
if not depistage_filtered.empty:
    plot_beneficiaries_by_categorie(
        df=depistage_filtered,
...
#### Nombre d'éligible pour la periode par agent {#eligible-global-office}
```{python, echo=FALSE}
...
#### Nombre d'éligible pour la periode par agent {#eligib-global-type}
```{python, echo=FALSE}
if not df_global_eligible.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
            df=df_global_eligible,
...
#### Nombre d'enrolés pour la periode par bureau {#enroled-global-office}
```{python, echo=FALSE}
...
#### liste des enfants enrolés pour la periode {#liste-enroled-global}
```{r, echo=FALSE}
display_table(py$enroled_global, "Enfants_enrolés_global", "#000080")
```


#### Nombre d'enfants à exeater par bureau {#child-to-exit-global}
```{python, echo=FALSE}
...
#### Liste des enfants à exeater pour la période sélectionnée {#liste-child-to-exit-global}
```{r, echo=FALSE}
display_table(py$child_to_exit_global, "Enfants_à_exeater_global", "#800000")
```

#### Nombre d'enfants exéatés pour la période sélectionnée par bureau {#exeat-global-office}
```{python, echo=FALSE}
exeat_global = enroled_global.loc[enroled_global['sortie']=='yes']
...
#### Liste des enfants exéatés pour la période sélectionnée {#liste-exeat-global}
```{r, echo=FALSE}
display_table(py$exeat_global, "Enfants_exéatés_global", "#000080")
```
#### Nombre d'enfants actifs pour la période sélectionnée  {#actif-global-agent}
```{python, echo=FALSE}
...
if not actif_global.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        eligible_col = "manutrition_type",
        set_title = "Répartition des actifs dans le programme par type de malnutrition",
...
#### Nombre d'enfants allaités pour la période sélectionnée  {#breasfeeding-global-agent}
```{python, echo=FALSE}
...
if not breasfeeding.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        eligible_col = "manutrition_type",
        set_title = "Répartition des enfants allaités dans le programme par type de malnutrition",
        set_xlabel = "Nombre d'enfants allaités",
        set_ylabel = "Type de malnutrition",
        horizontal = True,
        table = "no",
        percent = False,
...
#### Nombre d'enfants avec comptage de menage pour la période sélectionnée  {#comptage-global-agent}
```{python, echo=FALSE}
...
if not household.empty:
    plot_beneficiaries_stacked_eligible_matplotlib(
        ...
        eligible_col = "manutrition_type",
        set_title = "Répartition des enfants avec comptage de menage dans le programme par type de malnutrition",
...
#### Nombre d'enfants avec comptage de menage pour la période sélectionnée  {#liste-comptage-global-agent}
```{r, echo=FALSE}
display_table(py$no_household, "Enfants_sans_comptage_de_menage", "#800000")
```

#### Nombre d'enfants en club Nutrition pour la période sélectionnée par bureau {#club-nutrition-global-office}
:::{.card}
:::{.card-header}
📊 **Clubs de Nutrition - Analyse pour la période sélectionnée**
:::
...
if not club_global.empty:
    # Calcul du nombre d'enfants uniques par club
    children_uniques_global = club_global.groupby(['club_id', 'member_type', 'first_session_date', 'last_session_date', 'data_clerk']).agg(nombre_enfants_uniques=('caseid', 'nunique'))
    
    # Renommer la colonne 'caseid' en 'club_id' dans club_create si nécessaire
    if 'caseid' in club_create.columns:
        club_create = club_create.rename(columns={'caseid': 'club_id', 'departement': 'departement_club', 'commune': 'commune_club', 'office': 'office_club', 'name': 'name_club'})
    
    # Fusionner avec le DataFrame children_uniques_global et retirer les clubs de test
    clubs_a_exclure = ['Club Test', 'TEST ATELIER CARIS', 'Test Cap']
    children_by_global_club = children_uniques_global.reset_index().merge(club_create[['club_id','name_club','office_club','creation_date','departement_club','commune_club']], on='club_id', how='left')
    children_by_global_club = children_by_global_club[~children_by_global_club['name_club'].isin(clubs_a_exclure)]
    children_by_global_club.to_excel('outputs/NUTRITION/children_by_club.xlsx', index=False)
    ...
    if not children_by_global_club.empty:
        ...
        print("Pas de données disponibles après filtrage des clubs de test")
else:
    print("Pas de données disponibles pour la période sélectionnée")
...
if 'nombre_children_en_club_global' in globals() and nombre_children_en_club_global > 0:
    print(f"{nombre_children_en_club_global} enfants ont participé aux clubs de nutrition durant la période.\nEt {nombre_clubs_global} clubs étaient actifs.")
```
:::
:::
:::

#### Liste des enfants inscrits en club pour la période sélectionnée {#liste-children-by-club-global}
```{r, echo=FALSE}
display_table(py$children_by_global_club, "Enfants_en_club_global", "#800000")
```

#### Nombre d'enfants en attente pour la période par agent {#attente-global-agent}
```{python, echo=FALSE}
...
#### Liste des enfants en attente pour la période sélectionnée {#liste-attente-global}
```{r, echo=FALSE}
display_table(py$attente_global, "Enfants_en_attente_global", "#000080")
```

#### Récapitulatif de la période sélectionnée {#recapitulatif-global}

:::{.card}
:::{.card-header}
📈 **Tableau de bord - période sélectionnée**
:::
...
if summary_data:
    ...
    # Option 1 : Affichage simple avec print (texte brut)
    print("=== Récapitulatif de la période sélectionnée ===")
    #print(summary_df.to_string(index=False))
