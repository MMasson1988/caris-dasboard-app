...
# Configuration pour GitHub Actions : utiliser le chemin Python de l'environnement virtuel
reticulate::use_python(Sys.which("python"), required = TRUE)
# Installer python-dateutil si besoin
reticulate::py_install("python-dateutil", pip = TRUE)
```
...
```{r, echo=FALSE}
all_gardens <- read_excel("outputs/all_gardens.xlsx", sheet = 1)
```


```{python, echo=FALSE}
all_gardens = pd.read_excel("outputs/all_gardens.xlsx",sheet_name=0)
...
jardin_cases.to_excel("jardin.xlsx", index=False) 
#jardin_cases
```

```{r, echo = FALSE}
# plot
rall_gardens <- read_excel("outputs/all_gardens.xlsx", sheet = 1)
```
...
if all_graduated.empty:
    ...
    # Export en Excel
    all_graduated.to_excel("../outputs/all_graduated.xlsx", index=False)
...
if new_beneficiaires.empty:
    ...
    # Export en Excel
    new_beneficiaires.to_excel("../outputs/new_beneficiaires.xlsx", index=False)
...
if beneficiaires_cycle_3.empty:
    ...
    # Export en Excel
    beneficiaires_cycle_3.to_excel("../outputs/beneficiaires_cycle_3.xlsx", index=False)
